...
<div class="users">
    ...
    <div class="p-1">
        <div class="datatable-scroll" v-if="$can('list', 'user') || AuthenticatedUser.is_superadmin">
            <DataTable ref="usersDT" fetchMethod="fetchUsers" :columns="columns" cssClasses="table-card">
                ...
                <td slot="body-row" slot-scope="row">
                    <button v-if="($can('edit', 'user') || AuthenticatedUser.is_superadmin) && AuthenticatedUser.id !== row.data.original.id" class="btn icon-link" title="Reset user password" @click="resetUserPassword(row.data.original)">
                        <i class="fas fa-key"></i>
                    </button>
                    <button v-if="($can('edit', 'user') || AuthenticatedUser.is_superadmin) && AuthenticatedUser.id !== row.data.original.id" class="btn icon-link" title="Edit user" @click="editUser(row.data.original)">
                        <i class="fas fa-pen"></i>
                    </button>
                    <button v-if="($can('ban', 'user') || AuthenticatedUser.is_superadmin) && AuthenticatedUser.id !== row.data.original.id" class="btn icon-link" title="Ban user" @click="banUser(row.data.original)">
                    ...
                    <button v-if="($can('delete', 'user') || AuthenticatedUser.is_superadmin) && AuthenticatedUser.id !== row.data.original.id" class="btn icon-link" title="Delete user" @click="deleteUser(row.data.original)">
